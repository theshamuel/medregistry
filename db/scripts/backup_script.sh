...
(
    while :; do
        ...
        if [ "${MONGO_AUTH}" = true ]; then
            mongodump -h localhost -p $mongo_port --username $admin_login --password $admin_pass --authenticationDatabase admin --gzip --db medregDB --archive=$name_archive
        else
            mongodump -h localhost --gzip --db medregDB --archive=$name_archive
        fi
