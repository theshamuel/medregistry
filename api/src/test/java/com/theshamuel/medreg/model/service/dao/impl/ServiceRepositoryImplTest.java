...
public class ServiceRepositoryImplTest extends BaseRepositoryImplTest{
    ...
    public void testIsUniqueService() {
        Service service = new ServiceBuilder().label("service 1").price(BigInteger.valueOf(500)).build();

        Boolean actualTrue = serviceRepositoryImpl.isUniqueService("newLabel",BigInteger.valueOf(500));
        assertThat(actualTrue,is(Boolean.valueOf(true)));

        Boolean actualFalse = serviceRepositoryImpl.isUniqueService(service.getLabel(),BigInteger.valueOf(500));
    ...
    public void testFindPersonalRatesByDoctorId() {
        ...
        PersonalRate personalRate1 = new PersonalRateBuilder().price(BigInteger.valueOf(900)).doctorId("doc001").doctorPay(BigInteger.valueOf(100)).doctorPayType("sum").build();
        PersonalRate personalRate2 =new PersonalRateBuilder().price(BigInteger.valueOf(1900)).doctorId("doc001").doctorPay(BigInteger.valueOf(1100)).doctorPayType("sum").build();
        personalRates.add(personalRate1);
        personalRates.add(personalRate2);
        Service service4 = new ServiceBuilder().label("service 4").personalRates(personalRates).price(BigInteger.valueOf(5100)).build();
        template.save(service4);
        Service service5 = new ServiceBuilder().label("service 5").personalRates(personalRates).price(BigInteger.valueOf(5100)).build();
    ...
    public void createTestRecords() {
        ...
        template.save(new ServiceBuilder().label("service 1").price(BigInteger.valueOf(500)).build());
        template.save(new ServiceBuilder().label("service 2").price(BigInteger.valueOf(1000)).build());
        template.save(new ServiceBuilder().label("service 3").price(BigInteger.valueOf(200)).build());
    }
