...
public class DoctorRepositoryImplTest extends BaseRepositoryImplTest {
    ...
    public void testFindAllExcludeContractors() {
        List<Doctor> actual = doctorRepositoryImpl
                .findAllExcludeContractors(new Sort(new Sort.Order("ASC")));
    ...
    public void createTestRecords() {
        ...
        docSidorov = new DoctorBuilder().name("Alexey").surname("Sidorov").middlename("Ivanovich")
                .contractor(0).isNotWork(0).excludeFromReport(0).position("surgeon").build();
        docIvanova = new DoctorBuilder().name("Irina").surname("Ivanova").middlename("Ivanovna")
                .contractor(1).isNotWork(0).excludeFromReport(0).position("gynecologist").build();
        docOlegov = new DoctorBuilder().name("Oleg").surname("Olegov").middlename("Mihailovich")
                .contractor(1).isNotWork(0).excludeFromReport(0).position("nurse").build();
