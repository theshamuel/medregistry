...
public class ReportRepositoryImplTest extends BaseRepositoryImplTest{
    ...
    public void testIsUniqueReportServiceNotNull() {
        service = new ServiceBuilder().price(BigInteger.valueOf(500)).discount(BigInteger.valueOf(0)).label("Service 1").build();
        initCollection("services");
        template.save(service);
        service = template.findOne(Query.query(Criteria.where("label").is("Service 1").and("price").is(BigInteger.valueOf(500))),Service.class);
    ...
    public void testFindByService() {
        ...
        Report report = template.findOne(Query.query(Criteria.where("template").is("contract")), Report.class);
    ...
    public void testFindCommonReports() {
        ...
        createTestRecords();
        Report report = template.findOne(Query.query(Criteria.where("template").is("commonTemplate")), Report.class);
    ...
    public void createTestRecords() {
        ...
        template.save(new ReportBuilder().label("report 1").service(service).template("contract").build());
        template.save(new ReportBuilder().label("report 2").service(service).template("testTemplate").build());
        template.save(new ReportBuilder().label("report common").template("commonTemplate").build());
    }
