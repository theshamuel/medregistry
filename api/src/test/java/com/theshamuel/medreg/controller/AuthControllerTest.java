...
public class AuthControllerTest {
    ...
    public void testAuth() throws Exception {
        String login = "admin";
        User testUser =  new UserBuilder().id("0001").login(login).fullname("Senior admin").roles(createRoles()).password(Utils.pwd2sha256("123","salt")).salt("salt").isBlock(0).author("admin").build();
    ...
    public void testAuthNotFoundUser() throws Exception {
        ...
        mockMvc.perform(post("/auth").contentType(MediaType.APPLICATION_JSON_UTF8).content(TestUtils.convertObjectToJson(loginForm)))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.message",is("Пользователя с данным логином не существует")));
    ...
    public void testAuthLockedUser() throws Exception {
        String login = "admin";
        User testUser =  new UserBuilder().id("0001").login(login).fullname("Senior admin").roles(createRoles()).password(Utils.pwd2sha256("123","salt")).salt("salt").author("admin").build();
        AuthController.LoginForm loginForm = new AuthController.LoginForm();
        loginForm.login = login;
        loginForm.password = "123";
        ...
        mockMvc.perform(post("/auth").contentType(MediaType.APPLICATION_JSON_UTF8).content(TestUtils.convertObjectToJson(loginForm)))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.*",hasSize(2)))
                .andExpect(MockMvcResultMatchers.jsonPath("$.code",is(404)))
                .andExpect(MockMvcResultMatchers.jsonPath("$.message",is("Пользователь заблокирован")));
    ...
    public void testAuthWrongPassword() throws Exception{
        String login = "admin";
        User testUser =  new UserBuilder().id("0001").login(login).fullname("Senior admin").roles(createRoles()).password("no").salt("salt").isBlock(0).author("admin").build();
        AuthController.LoginForm loginForm = new AuthController.LoginForm();
        loginForm.login = login;
        loginForm.password = "123";
