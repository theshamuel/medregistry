...
public class ScheduleServiceImpl extends BaseServiceImpl<ScheduleDto, Schedule> implements
    ...
    public boolean findByDateWorkAndDoctor(String doctorId, LocalDate dateWork) {
        ...
        if (doctor != null) {
            return scheduleRepository.findByDateWorkAndDoctor(doctor, dateWork) != null;
        }
    ...
    public ScheduleDto copyPaste(String scheduleId) {
        ...
        if (schedule.isPresent()) {
            if (schedule.get().getDoctor() != null) {
                Optional<List<Schedule>> schedulesOfDoctor = Optional
                        .ofNullable(scheduleRepository.findByDoctor(schedule.get().getDoctor()));
                LocalDate newDateWork = LocalDate.now();
                if (schedulesOfDoctor.isPresent() && schedulesOfDoctor.get().size() > 0
                        && schedulesOfDoctor.get().get(0).getDateWork() != null) {
    ...
    public ScheduleDto obj2dto(Schedule schedule) {
        ...
        return new ScheduleDto(schedule.getId(), schedule.getCreatedDate(),
                schedule.getModifyDate(),
                schedule.getAuthor(), doctor, schedule.getDoctorLabel(), schedule.getDateWork(),
                schedule.getTimeFrom(),
                schedule.getTimeTo(), schedule.getBreakFrom(), schedule.getBreakTo(),
                schedule.getInterval(),
                schedule.getTimeFromOnline(), schedule.getTimeToOnline(),
                schedule.getIntervalOnline(),
    ...
    public Schedule dto2obj(ScheduleDto scheduleDto) {
        ...
        if (doctor != null) {
            return new Schedule(scheduleDto.getId(), scheduleDto.getCreatedDate(),
                    scheduleDto.getModifyDate(),
                    scheduleDto.getAuthor(), doctor, scheduleDto.getDateWork(),
                    scheduleDto.getTimeFrom(), scheduleDto.getTimeTo(), scheduleDto.getBreakFrom(),
                    scheduleDto.getBreakTo(), scheduleDto.getInterval(),
                    scheduleDto.getTimeFromOnline(), scheduleDto.getTimeToOnline(),
                    scheduleDto.getIntervalOnline());
        }
        return null;
    }
}
