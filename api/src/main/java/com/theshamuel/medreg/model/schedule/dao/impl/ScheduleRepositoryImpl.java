...
public class ScheduleRepositoryImpl  implements ScheduleOperations {
    ...
    public List<Schedule> findByDoctor(Doctor doctor) {
        ...
        Query query = Query.query(where).with(new Sort(new Sort.Order(Sort.Direction.DESC,"dateWork")));
    ...
    public List<Schedule> findByFilter(String filter) {
        ...
        if (params.length > 0) {
            ...
            for (int i = 0; i<params.length; i++){
                ...
                else if (!tmp[0].equals("doctor")) //Filter by doctor is implemented in ScheduleServiceImpl, due to complex relation. So that doctor parameter is excludes
                    where = where.and(tmp[0]).regex("^.*".concat(tmp[1].trim()).concat(".*$"),"i");
            }
            Query query = Query.query(where).with(new Sort(new Sort.Order(Sort.Direction.ASC,"dateWork"))).with(new Sort(new Sort.Order(Sort.Direction.ASC,"timeEvent")));
     ...
     */
    @Override
    public Schedule checkAppointmentInScheduleSlots(Doctor doctor, LocalDate dateEvent, LocalTime timeEvent) {
        Criteria where = Criteria.where("doctor").is(doctor).and("dateWork").is(dateEvent).and("timeFrom").lte(timeEvent).and("timeTo").gte(timeEvent);
        Query query = Query.query(where);
        return mongo.findOne(query, Schedule.class);
    }
