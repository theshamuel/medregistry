...
public class VisitServiceImpl extends BaseServiceImpl<VisitDto, Visit> implements VisitService {
     ...
     */
    @Autowired
    public VisitServiceImpl(VisitRepository visitRepository, DoctorRepository doctorRepository, ClientRepository clientRepository, AppointmentRepository appointmentRepository, ServiceRepository serviceRepository, ServiceService serviceService) {
    ...
    public boolean isUniqueVisit(String doctorId, String appointmentId) {
        Optional<Appointment> appointment = Optional.ofNullable(appointmentRepository.findOne(appointmentId));
        ...
        doctor.ifPresent(e->{
            ...
            result.forEach(item->{
                if (item.getServices()!=null) {
                    BigInteger totalSum = item.getServices().stream().reduce(BigInteger.valueOf(0), (res, p) -> {
                                BigInteger percentDiscount = (p.getDiscount() != null && p.getDiscount().compareTo(BigInteger.valueOf(0)) > 0) ? p.getDiscount() : BigInteger.valueOf(1);
                                if (percentDiscount.compareTo(BigInteger.valueOf(1)) > 0) {
                                    return res = res.add(BigInteger.valueOf(100).subtract(percentDiscount).multiply(p.getPrice()).abs().divide(BigInteger.valueOf(100)));
                                } else {
    ...
    public void addService(String visitId, String serviceId, BigInteger discount) {
        ...
        if (visit!=null && service !=null && doctor!=null) {
            ...
            service.setId(service.getId().concat("MEDREG").concat(String.valueOf(random.nextInt(Integer.MAX_VALUE))));
    ...
    public VisitDto save(VisitDto dto) {
        ...
        if (dto.getAppointmentId()!=null && dto.getAppointmentId().trim().length()>0) {
            if (dto.getId()!=null){
                Optional<Visit> editVisit = Optional.ofNullable(visitRepository.findOne(dto.getId()));
                editVisit.ifPresent(e->{
                    ...
                    Optional<Appointment> newAppointment =Optional.ofNullable( appointmentRepository.findOne(dto.getAppointmentId()));
                ...
                Optional<Appointment> appointment = Optional.ofNullable(appointmentRepository.findOne(dto.getAppointmentId()));
    ...
    public void delete(String id) {
        ...
        if (visit.getAppointment()!=null) {
            Optional<Appointment> appointment = Optional.ofNullable(appointmentRepository.findOne(visit.getAppointment().getId()));
            appointment.ifPresent(item -> {
                item.setHasVisit(false);
                appointmentRepository.save(item);
            });
    ...
    public VisitDto obj2dto(Visit visit) {
        ...
        String passportLabel = visit.getClient()!=null?visit.getClient().getPassportSerial().concat("/").concat(visit.getClient().getPassportNumber()):"-";
        String appointmentId = visit.getAppointment()!=null?visit.getAppointment().getId():null;

        return new VisitDto(visit.getId(),visit.getCreatedDate(),visit.getModifyDate(),
                visit.getAuthor(),visit.getContractNum(),doctorId, doctorLabel,clientId,visit.getServices(),visit.getTerminalSum(),BigInteger.valueOf(0), visit.getDateEvent(),visit.getTimeEvent(),clientLabel,appointmentId, visit.getDateTimeLabel(), visit.getTimeLabel(), phoneLabel,passportLabel);
    }
    ...
    public Visit dto2obj(VisitDto dto) {
        ...
        return new Visit(dto.getId(),dto.getCreatedDate(),dto.getModifyDate(),
                ...
                client,dto.getDateEvent(),dto.getTimeEvent(),appointment,dto.getServices(), dto.getTerminalSum());
    }
}
