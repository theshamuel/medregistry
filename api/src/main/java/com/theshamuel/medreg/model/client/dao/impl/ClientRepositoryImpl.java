...
public class ClientRepositoryImpl implements ClientOperations {
    ...
    public boolean isUniqueClient(Client client) {
        ...
        Criteria where = Criteria.where("passportSerial").is(passportSerial).and("passportNumber").is(passportNumber);
        Query query = Query.query(where);
        if (mongo.findOne(query, Client.class)!=null)
            return false;
        return true;
    }
        ...
        if (params.length > 0) {
            ...
            for (int i = 0; i<params.length; i++){
                ...
                if (tmp[0].equals("passport")) {
                    where = where.orOperator(Criteria.where("passportSerial").regex("^.*".concat(tmp[1].trim()).concat(".*$"),"i"),Criteria.where("passportNumber").regex("^.*".concat(tmp[1].trim()).concat(".*$"),"i"));
                }else
