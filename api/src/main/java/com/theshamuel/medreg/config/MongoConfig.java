...
public class MongoConfig extends AbstractMongoConfiguration {
    ...
    public Mongo mongo() throws Exception {
        ...
        if (environment.getProperty("MONGO_SERVER") != null && !environment
                .getProperty("MONGO_SERVER").trim().equals("")) {
            MONGO_SERVER = environment.getProperty("MONGO_SERVER");
        } else {
            logger.warn("Doesn't set up $MONGO_SERVER");
        }

        if (environment.getProperty("MONGO_PORT") != null && !environment.getProperty("MONGO_PORT")
                .trim().equals("")) {
        ...
        if (environment.getProperty("MONGO_DB") != null && !environment.getProperty("MONGO_DB")
                .trim().equals("")) {
        ...
        if (environment.getProperty("MONGO_USER") != null && !environment.getProperty("MONGO_USER")
                .trim().equals("")) {
        ...
        if (environment.getProperty("MONGO_USER_PASSWORD") != null && !environment
                .getProperty("MONGO_USER_PASSWORD").trim().equals("")) {
        ...
        if (MONGO_USER != null && MONGO_PASSWORD != null) {
            return new MongoClient(singletonList(new ServerAddress(MONGO_SERVER, MONGO_PORT)),
                    singletonList(MongoCredential
                            .createCredential(MONGO_USER, MONGO_DB, MONGO_PASSWORD.toCharArray())));
        } else {
